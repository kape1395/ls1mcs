...
-define(REF(CmdFrameId), {via, gproc, {?MODULE, CmdFrameId}}).
-define(DATA_TIMEOUT, 600000).  % 10 min
...
%%
received(Ack = #ls1p_ack_frame{cref = FrameId}) ->
    gen_fsm:send_event(?REF(FrameId), {recv, Ack});

received(Data = #ls1p_data_frame{cref = FrameId}) ->
    gen_fsm:send_event(?REF(FrameId), {recv, Data}).
...
-record(state, {
    sat_cmd,        %% SAT command to send.
    usr_cmd_ref,     %% User command ID, that initiated this command.
    ls1p_ref        %% Protocol ref to send frame to.
}).
...
sending(start, StateData = #state{sat_cmd = SatCmd, ls1p_ref = Ls1pRef}) ->
    #ls1p_cmd_frame{ack = NeedsAck, delay = Delay} = SatCmd,
    ok = ls1mcs_protocol:send(Ls1pRef, SatCmd),
    _TRef = gen_fsm:send_event_after(Delay * 1000 + ?DATA_TIMEOUT, timeout),
    case {NeedsAck, needs_data(SatCmd)} of
...
waiting_ack({recv, #ls1p_ack_frame{status = Status}}, StateData) ->
    #state{sat_cmd = SatCmd = #ls1p_cmd_frame{cref = SatCmdId}, usr_cmd_ref = UsrCmdRef} = StateData,
...
waiting_ack(timeout, StateData) ->
    #state{sat_cmd = #ls1p_cmd_frame{cref = SatCmdId}, usr_cmd_ref = UsrCmdRef} = StateData,
    ok = ls1mcs_usr_cmd:sat_cmd_failed(UsrCmdRef, SatCmdId),
    {stop, normal, StateData}.
